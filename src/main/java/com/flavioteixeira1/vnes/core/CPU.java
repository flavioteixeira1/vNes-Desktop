...
public final class CPU implements Runnable{
	...
	private static final long NANOS_PER_FRAME = 1000000000L; // TARGET_FPS; // ~16.666ms em n
	...
	public synchronized void endExecution(){
		//System.out.println("CPU.endExecution() - Solicitando parada...");
		stopRunning = true;
		// Notificar a thread se estiver esperando
    synchronized (this) {
        this.notifyAll();
    }
    
    // Esperar um pouco pela thread terminar
    try {
        Thread.sleep(100);
    } catch (InterruptedException e) {
       // System.err.println("Interrupção ao parar thread da CPU: " + e.getMessage());
    }
   // System.out.println("CPU parada.");
	...
	public void emulate(){
		...
		stopRunning = false;
		 ...
		 // CONTADOR PARA SINCRONIZAÇÃO DE 60 FPS
        int cycleCounter = 0;
        int cyclesPerFrame = 29788; // NTSC ~60 FPS
		long lastSyncTime = System.nanoTime();
		long syncInterval = NANOS_PER_FRAME;
		...
		while(true){
			...
			// ... (código existente de verificação de IRQ)

            opinf = opdata[mmap.load(REG_PC+1)];
            cycleCount = (opinf>>24); // ← cycleCount É DEFINIDA AQUI
            cycleAdd = 0;


			 // SINCRONIZAÇÃO COM ÁUDIO A CADA FRAME
            cycleCounter += cycleCount;
            if (cycleCounter >= cyclesPerFrame) {
                //syncWithAudio();
                cycleCounter = 0;
                
                // Pequena pausa para manter 60 FPS
				/*
                try {
                    Thread.sleep(1);
                } catch (InterruptedException e) {
			...
			if(emulateSound){
				...
				if (cycleCount % 50 == 0) {
					try {
						Thread.sleep(0, 1000); // 1 microssegundo
					} catch (InterruptedException e) {
						Thread.currentThread().interrupt();
						break;
					}
    ...
    private void syncWithAudio() {
        // SINCRONIZAÇÃO NÃO-BLOQUEANTE COM ÁUDIO
        PAPU papu = nes.getPapu();
        if (papu != null) {
            int bufferUsage = (papu.getBufferIndex() * 100) / papu.getPapuBufferSize();
            
            // Se o buffer estiver muito cheio, pausa mínima
            if (bufferUsage > 80) {
                try {
                    Thread.sleep(1);
                } catch (InterruptedException e) {
